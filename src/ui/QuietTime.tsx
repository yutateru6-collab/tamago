...
import { REST_EVENTS, restEventState } from '../domain/restEvents';
export function QuietTime({world, busy, complete, cancel, back}: {world:World; busy:boolean; complete:()=>void; cancel:()=>void; back:()=>void}) {
  ...
  return <section className="quiet-screen">
    ...
    </div><p className="quiet-reassurance">画面を閉じて、大丈夫。<br/>戻ったら、つづきから見られます。<br/>結果は「休めた」と伝えたときに記録します。</p><p className="quiet-tip">{new Date(session.endsAt).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'})} ごろに、またね <span>· 通知は届きません</span></p></>:<><button className="primary" disabled={busy} onClick={complete}>30分、スマホを休めた</button><p className="honesty-note">自己申告として記録します。次の画面で、元気や制作・お手入れの変化を確認できます。</p></>}
