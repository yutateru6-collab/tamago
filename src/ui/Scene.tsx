...
export function Scene({ world, onSettings }: { world: World; onSettings?: () => void }) {
  ...
  return <section className={`scene ${condition}`} aria-label={`住処の様子：${label}`}>
    ...
    <header className="scene-header"><p className="eyebrow">ひと休みから、暮らしが育つ。</p><h1 tabIndex={-1}>こもれびの巣</h1></header>
    ...
    <span className="scene-furniture-count" aria-label={`完成した家具 ${world.built.length}個`}>手づくり {world.built.length} / 3</span>
  </section>;
}
