...
export default function Prototype() {
  ...
  return <div className="tamago">
    <MobileScroll key={away?'away':tab} className="app-screen"><main className="tamago-content">
      ...
      : tab==='habitat'?<Habitat world={app.world} busy={blocked} onAway={openQuiet} onExplore={()=>setTab('explore')} craft={id=>void act(async()=>{if(await app.craft(id))setTab('home');})}/>
    ...
    <BottomSheet open={settings} onOpenChange={setSettings} title="この子との暮らし方" description="自己申告で、少しずつ暮らしを育てます。" snap={0.82}>
      <div className="sheet-body"><h3>つい、使いすぎたとき</h3><p>必要な連絡や仕事は気にしなくて大丈夫。自分で「30分、余分に見てしまった」と感じたときだけ記録できます。元気と住処が少し下がりますが、休めば戻ります。</p><button className="secondary" disabled={blocked} onClick={()=>void act(app.reportUsage)}>使いすぎ30分を記録する</button><h3>変化をすぐに試す</h3><p>ここでは仮の時間で、キャラと住処の変化を試せます。アプリを閉じただけでは、回復したことにしません。</p>
        <div className="demo-actions"><button className="primary" disabled={blocked} onClick={()=>void act(()=>app.simulate('away',120))}>離れた時間を試す（2時間）</button>
        <button className="secondary" disabled={blocked} onClick={()=>void act(()=>app.simulate('usage',120))}>使いすぎた状態を試す（2時間）</button></div>
