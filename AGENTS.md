...
## Miniature 2.5D home direction — 2026-09-25

The user asks to implement the supplied warm miniature-diorama visual direction. Keep this a layered 2D home: one unfurnished base scene, the approved original blue companion still/video, and separately earned furniture at stable positions. Retain the current progress, rest reporting, wear, storage, and developer sandbox rules. The supplied squirrel is a style reference, not an authorized replacement for the companion. Measuring whether the phone was actually put down remains deferred; do not label elapsed browser time as detected phone inactivity.
